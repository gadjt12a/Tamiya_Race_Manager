...
- [x] ~~**Upgrade over an existing v9.3x install**~~ — **dropped 2026-08-09,
      not deferred.** No v9.3x ever reached anyone: the only tag is `v9.27`
      (zip-style) and there has never been a GitHub release, so no machine
      outside this repo has an installed exe for a new installer to land on.
      Confirmed by Kris. The scenario is unreachable, so it cannot be tested
      and does not need to be. **This will stop being true the moment v10.0
      ships** — installer-over-installer becomes the normal upgrade path for
      every club from then on, and needs testing before v10.1.
...
| ~~**Upgrading over an existing install is untested.**~~ **Withdrawn 2026-08-09:** nobody has an existing install. The only tag is `v9.27` (zip-style), there has never been a GitHub release, and v9.3x was never distributed — so `{app}` going from a lone exe to exe + `_internal\` is a transition no real machine will make. | Nobody | None needed. **Reinstate this the day v10.0 ships**, when installer-over-installer becomes every club's upgrade path. |
